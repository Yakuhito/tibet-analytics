...
export default function Navbar({ theme, setTheme }: NavbarProps) {
  ...
  return (
    <>
      <header className="sticky w-full top-0 bg-brandLight/50 dark:bg-zinc-900/50 backdrop-blur-xl h-24">
        <div className="container mx-auto px-4 flex gap-4 sm:gap-8 items-center justify-between py-4">
          ...
          <nav className="hidden md:flex items-center rounded-xl p-1">
            <Link href={`${process.env.NEXT_PUBLIC_APP_URL}`} className="hover:opacity-80 font-medium text-brandDark/70 dark:text-brandLight/50 px-6 py-1.5 rounded-xl">Trade</Link>
      ...
      <footer className="md:hidden fixed bottom-0 left-0 w-full bg-brandLight/50 dark:bg-zinc-900/50 backdrop-blur-xl z-20 p-4">
        <nav className="flex items-center rounded-xl p-1 w-full text-center justify-around">
          <Link href={`${process.env.NEXT_PUBLIC_APP_URL}`} className="hover:opacity-80 font-medium text-brandDark/70 dark:text-brandLight/50 px-6 py-1.5 rounded-xl">Trade</Link>
          <Link href={`${process.env.NEXT_PUBLIC_APP_URL}/faq`} className="hover:opacity-80 font-medium text-brandDark/70 dark:text-brandLight/50 px-6 py-1.5 rounded-xl">FAQs</Link>
          <Link href="/" className="font-medium text-brandDark dark:text-brandLight px-6 py-1.5 bg-brandDark/10 rounded-xl">Stats</Link>
        </nav>
