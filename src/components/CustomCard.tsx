...
export const CustomCard: React.FC<CustomCardProps> = ({ title, value, subtitle }) => {
  return (
    <div>
      ...
      <p className="text-3xl xl:text-4xl font-bold text-white font-mono px-4 flex flex-col">{value}<span className="text-sm font-normal text-brandLight/90 whitespace-nowrap">{subtitle}</span></p>
  </div>
  );
};
